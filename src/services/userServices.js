...
const baseUrl = "https://localhost:9050/api/UserService";
...
export const verifyOtp = async (phoneNumber, otp ,verificationId , chatId ,isUpdatePhone , oldPhoneNumber) => {
  try {
...
export const fetchOfferByFilterNoToken = async ({
  ...
  try {
    ...
    const normalizedCategory = category ? categoryMapping[category] || category : category;
    ...
    const response = await axios.post(`${baseUrl}/dynamicOfferFilterNoToken`, payload, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const fetchOfferByFilter = async ({
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/dynamicOfferFilter`, payload, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const fetchUser = async ({ chat_id, contact_information }) => {
  try {
    ...
    if (contact_information) queryParams.append("contact_information", contact_information);
    
    const response = await axios.get(`${baseUrl}/fetchuser?${queryParams.toString()}`);
...
export const addUser = async ({ phoneNumber, chatId , fullName}) => {
  try {
    const response = await axios.post(`${baseUrl}/adduser`, {
      contact_information: phoneNumber, chat_id :chatId , name:fullName
    })
  } catch (error) {
